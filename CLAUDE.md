...
- **Hold-Liste:** `~/.claude/skills/.sync-hold` markiert Skills, deren Deployment-Version bewusst lokal abweicht (lokale Forks, z.B. Therapy-Einstiegs-Skills mit Routing-Tabellen). Sie werden beim Sammel-Deploy übersprungen; explizites Deploy braucht `--force`. Die drei Therapy-Einstiegs-Skills (`counseling-basics`, `guideline-therapies-overview`, `stabilization-techniques`) sind dauerhafte Forks (Entscheid 2026-06-13): Repo = publizierte Variante, Deployment = lokale Routing-Variante — eine Konsolidierung ist nicht geplant.
