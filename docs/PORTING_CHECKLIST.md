...
- [ ] **BACH-Export nutzen (wenn moeglich)**:
      `python skill_export.py --skill <name> --format anthropic --output <USER_HOME>/OneDrive/.SKILLS/skills/<kategorie>/`
...
- [ ] **Python-Code bereinigen:**
      ...
      - Hardcodierte Pfade (`<USER_HOME>\...`) → relative Pfade
...
- [ ] **Grep-Pruefung** -- Keine verbotenen Muster:
      ...
      rg "C:\\\\Users|<USER_HOME>" SKILL.md scripts/
      ```
...
| `<USER_HOME>\` | Nutzerspezifisch | Relative Pfade |
...
```bash
cd <USER_HOME>/OneDrive/.AI/BACH/system
PYTHONIOENCODING=utf-8 python tools/skill_export.py \
  --skill <skill-name> \
  --format anthropic \
  --output <USER_HOME>/OneDrive/.SKILLS/skills/<kategorie>/
```
