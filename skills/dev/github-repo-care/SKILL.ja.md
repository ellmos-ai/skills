...
14. **発見用サーフェスを更新する。** 組織プロファイル、`llms.txt`、中央レジストリ、ローカルモジュールインデックス、およびエコシステム README からリンクします。**公開 ⇒ プロファイルページ（T-20260926-796851315）:** リポジトリにバナーがある場合、組織のプロファイルページ（`<org>/.github/profile/README.md`）に既に掲載されているか機械的に確認する — `python org_profile_gate.py --org <org>`（`.AI/.SKILLS/testing/`）。欠けていれば、スクリプトが貼り付け可能な Markdown スニペットを生成します。
...
- [ ] バナーがある場合、`org_profile_gate.py --org <org>` を実行してこのリポジトリで検出なしを確認した。

## 変更履歴

### 1.1.0 (2026-09-26)
- 「公開 ⇒ プロファイルページ」ステップを追加（T-20260926-796851315）：バナーを持つリポジトリは、
  組織のプロファイル README（`<org>/.github/profile/README.md`）と照合する必要があり、記憶に頼らず
  `.AI/.SKILLS/testing/org_profile_gate.py` で機械的に確認する。契機：zombie-killer-tray の
  バナーが公開後 dev-bricks の組織ページに反映されていなかった。
