...
14. **Обновить страницы обнаружения.** Разместите ссылки в профиле организации, `llms.txt`, центральных реестрах, локальных индексах модулей и README экосистемы. **Публикация ⇒ страница профиля (T-20260926-796851315):** если у репозитория есть баннер, механически проверьте, упомянут ли он уже на странице профиля организации (`<org>/.github/profile/README.md`) — `python org_profile_gate.py --org <org>` (`.AI/.SKILLS/testing/`). Если отсутствует, скрипт выдаёт готовый markdown-фрагмент для вставки.
...
- [ ] Есть баннер? `org_profile_gate.py --org <org>` выполнен без находок для этого репозитория.

## Журнал изменений

### 1.1.0 (2026-09-26)
- Добавлен шаг «Публикация ⇒ страница профиля» (T-20260926-796851315): репозиторий с баннером
  должен проверяться по README профиля организации (`<org>/.github/profile/README.md`)
  механически через `.AI/.SKILLS/testing/org_profile_gate.py`, а не полагаясь на память. Повод:
  баннер zombie-killer-tray так и не появился на странице организации dev-bricks после публикации.
