...
"""
nulcleaner.py - Windows NUL-Datei Bereinigung

Findet und loescht Windows-reservierte NUL-Dateien, die durch
/dev/null-Verwendung in Git Bash entstehen.

Modi:
    scan   - Verzeichnis rekursiv nach NUL-Dateien durchsuchen
    delete - NUL-Dateien finden und loeschen
    gui    - Grafische Oberflaeche

Usage:
    python nulcleaner.py scan <pfad>
    python nulcleaner.py delete <pfad>
...
class NulCleanerGUI:
    def __init__(self, root):
        ...
        tk.Button(root, text="Auswaehlen", command=self.select_dir).pack(side="left")
        ...
        tk.Button(root, text="Ausgewaehlte loeschen", command=self.delete_selected).pack(pady=10)
    ...
    def scan(self):
        ...
        if not os.path.isdir(path):
            messagebox.showerror("Fehler", "Ungueltiges Verzeichnis")
            return
    ...
    def delete_selected(self):
        ...
        if not selected:
            messagebox.showwarning("Hinweis", "Keine Datei ausgewaehlt")
            return
        ...
        for idx in reversed(selected):
            ...
            if ok is True:
                ...
                messagebox.showerror("Fehler", f"Konnte {path} nicht loeschen")

        messagebox.showinfo("Fertig", "Loeschvorgang abgeschlossen")
