...
language: zh
description: >
  针对政治与决策的无偏见道德-法律审查机构 —— 研究项目“未出生者的立场”（阴影模式第1阶段）的可运行原型。每当需要分析、审查或评估一项政治决策、法律（草案）、改革、预算决议或社会争议问题时使用此 Skill —— 也包括诸如“从后代视角审查”、“法律通行证”、“基本法对此怎么说”、“叠加审查”、“立法历史/容器分析”、“影响评估”、“分析这项改革”、“活的宪法”等表述，或者当用户提出政治问题并要求提供中立的多阶段评估时。编排 5-CORE 架构（config.json）：道德叠加实例、法典化身、两阶段影响评估（具有证据层级的回顾性/前瞻性）、具有本地存储的知识句柄、可配置的工作流。
---
...
## 五大 CORE 架构（分工：适用什么 · 生效什么 · 如何获取 · 何时）
